const { Client, Storage } = require('node-appwrite');

const client = new Client()
    .setEndpoint('https://sfo.cloud.appwrite.io/v1')
    .setProject('69d59db800358cca9f27')
    .setKey('standard_');

const storage = new Storage(client);
const BUCKET_ID = 'images_bucket';

async function updateBucket() {
    try {
        console.log('Ensuring JPG, PNG, WEBP are fully enabled (including uppercase versions)...');
        const result = await storage.updateBucket(
            BUCKET_ID,
            'Imagens Globais',
            ['read("any")', 'create("users")', 'update("users")', 'delete("users")'],
            false,
            true,
            50000000,
            ['jpg', 'JPG', 'jpeg', 'JPEG', 'png', 'PNG', 'webp', 'WEBP', 'svg', 'SVG', 'gif', 'heic', 'HEIC', 'mp4', 'mov', 'webm'],
            'none',
            true,
            true
        );
        console.log('Bucket Settings Updated Successfully!');
        console.log('Allowed Extensions:', result.allowedFileExtensions.join(', '));
    } catch (error) {
        console.error('Error updating bucket:', error.message);
    }
}

updateBucket();

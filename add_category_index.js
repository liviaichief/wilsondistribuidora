
import { Client, Databases } from 'node-appwrite';

const ENDPOINT = 'https://sfo.cloud.appwrite.io/v1';
const PROJECT_ID = '698e695d001d446b21d9';
const API_KEY = 'standard_';
const DATABASE_ID = 'boutique_carne_db';
const PRODUCTS_COLLECTION = 'products';

const client = new Client()
    .setEndpoint(ENDPOINT)
    .setProject(PROJECT_ID)
    .setKey(API_KEY);

const databases = new Databases(client);

async function addCategoryIndex() {
    console.log(`Adding index to 'category' in ${PRODUCTS_COLLECTION}...`);
    try {
        // Create Key index on 'category'
        await databases.createIndex(
            DATABASE_ID,
            PRODUCTS_COLLECTION,
            'idx_category',
            'key',
            ['category'],
            ['ASC']
        );
        console.log("Index 'idx_category' creation initiated.");
    } catch (error) {
        console.error("Error creating index:", error.message);
    }
}

addCategoryIndex();

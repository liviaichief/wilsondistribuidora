const { Client, Databases } = require('node-appwrite');

const client = new Client()
    .setEndpoint('https://sfo.cloud.appwrite.io/v1')
    .setProject('69d59db800358cca9f27')
    .setKey('standard_');

const databases = new Databases(client);

async function check() {
    const products = await databases.listDocuments('main_db', 'products');
    products.documents.forEach(p => {
        // Se a imagem for uma URL do Unsplash ou nula/vazia
        const needsImage = !p.image || p.image.includes('unsplash') || p.image.includes('images.com');
        if (needsImage) {
            console.log(`MISSING: [${p.$id}] ${p.title} (Current: ${p.image})`);
        }
    });
}

check();

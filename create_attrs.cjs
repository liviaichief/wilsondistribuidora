const { Client, Databases } = require('node-appwrite');

const client = new Client()
    .setEndpoint('https://sfo.cloud.appwrite.io/v1')
    .setProject('69d59db800358cca9f27')
    .setKey('standard_');

const databases = new Databases(client);
const DB_ID = 'main_db';
const COL_ID = 'products';

async function checkAndCreate() {
    try {
        console.log("Creating float 'stock'");
        await databases.createFloatAttribute(DB_ID, COL_ID, 'stock', false);
    } catch(e) { console.log("stock exists or error", e.message); }
    
    try {
        console.log("Creating float 'stock_alert'");
        await databases.createFloatAttribute(DB_ID, COL_ID, 'stock_alert', false);
    } catch(e) { console.log("stock_alert exists or error", e.message); }
}
checkAndCreate();

const { Client, Databases } = require('node-appwrite');

const client = new Client()
    .setEndpoint('https://sfo.cloud.appwrite.io/v1')
    .setProject('69d59db800358cca9f27')
    .setKey('standard_');

const databases = new Databases(client);

async function reclassify() {
    try {
        const response = await databases.listDocuments('main_db', 'products');
        const products = response.documents;

        const meatKeywords = ['bovinos', 'suinos', 'suínos', 'aves', 'carne', 'frango', 'pork'];
        const kitKeywords = ['kit', 'combo'];

        for (const p of products) {
            const categoryMatch = p.category.toLowerCase();
            const titleMatch = p.title.toLowerCase();

            const isMeat = meatKeywords.some(k => categoryMatch.includes(k) || titleMatch.includes(k));
            const isKit = kitKeywords.some(k => categoryMatch.includes(k) || titleMatch.includes(k));

            if (!isMeat && !isKit) {
                console.log(`Moving to Mercado: ${p.title} (Old Cat: ${p.category})`);
                await databases.updateDocument('main_db', 'products', p.$id, {
                    category: 'Mercado'
                });
            } else {
                console.log(`Keeping: ${p.title} (Cat: ${p.category})`);
            }
        }
        console.log('Reclassification complete!');
    } catch (e) {
        console.error('Error:', e.message);
    }
}

reclassify();
